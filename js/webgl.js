...
function outputWebGL() {
	...
	function get_webgl() {
		...
		Promise.all([
			...
			// perf
			if (logPerf) {debug_log("properties [webgl]",t0)}
		})
	}

	function get_webgl_render() {
		let t1 = performance.now()
		let res1 = "", res2 = ""

		function display_readpixels() {
			let pushvalue = res1
			if (res1 !== res2) {
				pushvalue = "random"
			...
			// perf
			if (logPerf) {debug_log("render [webgl]",t1)}
			debug_page("perf","webgl",t0,gt0)
